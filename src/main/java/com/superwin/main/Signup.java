...
//	WebElement enterAffiliateCode;
	...
	@FindBy(xpath = "//div[@class = 'v-card__text pa-12']//button//span[contains(text(),'Register')]")
	WebElement register;
	...
	public void checkURL(String reqBody) throws Exception {
		String affiliatecode = null;
		String url = driver.getCurrentUrl();
		extentTest.info("URL : " + url);
		if(url.contains("btag")) {
			int index = url.indexOf("=") + 1;
			affiliatecode = url.substring(index);
			affiliateCodeIdentification();
		} else {
			affiliatecode = affiliateCode(reqBody);
		}
		this.affiliatecode = affiliatecode;
	}

	public void affiliateCodeIdentification() {
		if (!affiliatecode.equals(null)) {
			...
			extentTest.info("Affiliate Code Entered In Affiliate Field Manually..!!!");
		}
	}
	...
	public String affiliateCode(String reqBody) throws Exception {
		String apiUrl = GetGsonTestData.getConfigData().getUserAPIUrl();
